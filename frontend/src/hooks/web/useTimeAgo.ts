...
const TIME_AGO_MESSAGE_MAP: {
  ...
  'zh-CN': {
    ...
    past: (n) => (n.match(/\d/) ? `${n}forward` : n),
    future: (n) => (n.match(/\d/) ? `${n}back` : n),
    month: (n, past) => (n === 1 ? (past ? 'last monnext month' : 'next month') : `${n} months`),
    year: (n, past) => (n === 1 ? (past ? 'last year' : 'next year') : `${n} Year`),
    day: (n, past) => (n === 1 ? (past ? 'yesterday' : 'tomorrow') : `${n} sky`),
